...
let setVolume = (volume       ) => { throw new Error('Set volume not implemented'); }; // eslint-disable-line no-unused-vars
let getVolume = () => { throw new Error('Get volume not implemented'); };
let setMuted = (muted        ) => { throw new Error('Set muted not implemented'); }; // eslint-disable-line no-unused-vars
...
const useLoudness = () => {
  ...
  setVolume = (volume        ) => loudness.setVolume(volume);
  getVolume = () => loudness.getVolume();
  setMuted = (muted         ) => loudness.setMuted(muted);
...
const useWinAudio = () => {
  ...
  setVolume = (volume        ) => winAudio.setVolume(volume / 100);
  getVolume = () => winAudio.getVolume() * 100;
  setMuted = (muted         ) => winAudio.setMute(muted);
