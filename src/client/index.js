...
export default class Client extends EventEmitter {
  ...
  constructor(element: HTMLVideoElement, address:string) {
    super();
    const urlMatch = address.match(urlRegex);
    if(!urlMatch || !urlMatch[1]) {
      throw new Error(`Invalid address ${address}`);
    }
    const streamUrl = decodeURIComponent(urlMatch[1]);
    this.videoLogger = makeLogger(`${streamUrl} Video Element`);
    this.mediaSourceLogger = makeLogger(`${streamUrl} Media Source`);
    this.videoBufferLogger = makeLogger(`${streamUrl} Video Source Buffer`);
    this.audioBufferLogger = makeLogger(`${streamUrl} Audio Source Buffer`);
    this.websocketLogger = makeLogger(`${streamUrl} WebSocket`);
   ...
   * @return {Promise<void>}
   */
  async openWebSocket(address:string) {
