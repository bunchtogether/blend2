...
module.exports.getPairRouter = () => {
  ...
  router.post('/api/1.0/pair/discover', async (req: express$Request, res: express$Response) => {
    const body = req.body;
  ...
  router.post('/api/1.0/pair/start', async (req: express$Request, res: express$Response) => {
    const body = req.body;
    if (!body || typeof body !== 'object') {
      res.status(400).send('Missing request body');
      return;
  ...
  router.post('/api/1.0/pair', async (req: express$Request, res: express$Response) => {
    const body = req.body;
    if (!body || typeof body !== 'object') {
      res.status(400).send('Missing request body');
      return;
  ...
  router.get('/api/1.0/pair', async (req: express$Request, res: express$Response) => {
    const activeAdapter = adapters.getActiveAdapter();
